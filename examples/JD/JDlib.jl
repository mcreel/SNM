...
function auxstat(θ::Array{Float64}, reps::Int64)
    ...
    while not_ok
        ...
        stats = auxstat(rets, RV, BV, reps)
        not_ok = any(isnan.(stats))
        if not_ok println("NaN in stats") end
    end
    return stats
end
...
function auxstat(rets::Array{Float64}, RV::Array{Float64}, BV::Array{Float64}, reps::Int64)
    ...
    @inbounds Threads.@threads for rep = 1:reps
        ...
        BVr = BV[rep*n-n+1:rep*n]
        jumpr = jump[rep*n-n+1:rep*n]
        nojumpr = nojump[rep*n-n+1:rep*n]
        # ensure variation
        jumpr[1:2] .= true
        nojumpr[1:2] .= true
        # jump stats 
        jumpsize = mean(RVr[jumpr]) - mean(BVr[jumpr])
        jumpsize2 = std(retsr[jumpr]) - std(retsr[nojumpr])
        njumps = mean(jumpr[3:end])
        # ρ
        X = [ones(n-1) BVr[2:end] BVr[1:end-1]]
        y = retsr[2:end]
        βrets = X\y
        ϵrets = y-X*βrets
        σrets = std(ϵrets)
        ...
        X = [ones(n-2) BVr[1:end-2] BVr[2:end-1]]
        y = BVr[3:end]
        βvol = X\y
        ϵvol = y-X*βvol
        σvol = std(ϵvol)
